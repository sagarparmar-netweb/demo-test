#!/usr/bin/env python3
"""
VFinal2 - 50 Sample Validation Test

Run this before production to verify pipeline accuracy at scale.
Uses fixed random_state=123 for reproducible results.

Usage:
    export HF_API_TOKEN="your_token_here"
    python tests/test_50_samples.py
"""
...
NUM_SAMPLES = 50
OUTPUT_CSV = PROJECT_DIR / 'reports' / 'vfinal2-test50.csv'
...
def main():
    ...
    print("=" * 80)
    print("VFINAL2 - 50 SAMPLE VALIDATION TEST")
    print("=" * 80)
    ...
    for i, (idx, row) in enumerate(samples.iterrows(), 1):
        ...
        if i % 10 == 0 or i == NUM_SAMPLES:
            ...
            print(f"Progress: {i}/{NUM_SAMPLES} | CPT: {curr_cpt_acc:.0%} | MOD: {curr_mod_acc:.0%} | ICD Recall: {curr_icd_recall:.0%}")
    ...
    if cpt_acc >= 0.95 and mod_acc >= 0.95 and avg_icd_recall >= 0.90:
        print("✅ TESTS PASSED - Ready for production!")
        return 0
    else:
        print("⚠️ REVIEW NEEDED - Some metrics below threshold")
        if cpt_acc < 0.95:
            print(f"   - CPT accuracy {cpt_acc:.1%} < 95%")
        if mod_acc < 0.95:
            print(f"   - Modifier accuracy {mod_acc:.1%} < 95%")
        if avg_icd_recall < 0.90:
            print(f"   - ICD recall {avg_icd_recall:.1%} < 90%")
        return 1
